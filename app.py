...
def gui():
    ...
    root = tk.Tk()
    root.title('Image → Radiant | Brush prefab workshop')
    ...
    ttk.Label(header,text='IMAGE → RADIANT',font=('Segoe UI',20,'bold')).pack(anchor='w')
    ttk.Label(header,text='Silhouette to editable CoD4 brushes • local processing • iwmap 4').pack(anchor='w')
    ...
    for key,label,choices in [
        ...
        ('resolution','Longest edge (4–512 cells)',None),
        ('threshold','Threshold (0–255)',None),
    ...
    def render():
        ...
        status.set(f'{state["path"].name} • {len(boxes):,} brushes • image plane {width:g} × {height:g} units • depth {s.depth:g}. Preview uses current settings.')
        return s,mask,boxes
    def guarded(fn):
        def run():
            try: fn()
                ...
                messagebox.showerror('Image → Radiant',str(e))
        return run
    def load(path=None):
    ...
    ttk.Button(controls,text='Open image…',command=guarded(load)).pack(fill='x',pady=(18,4))
    ttk.Button(controls,text='Load honeycomb example',command=guarded(lambda:load(Path(__file__).parent/'examples'/'honeycomb.png'))).pack(fill='x',pady=4)
    ttk.Button(controls,text='Update preview',command=guarded(render)).pack(fill='x',pady=4)
    ttk.Button(controls,text='Export .map…',command=guarded(save)).pack(fill='x',pady=4)
