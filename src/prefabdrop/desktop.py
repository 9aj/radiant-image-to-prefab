...
def main():
    ...
    root=tk.Tk(); root.title('Image â†’ Radiant | Prefab workshop'); root.geometry('1200x780')
    ...
    ttk.Label(root,text='IMAGE â†’ RADIANT',font=('Segoe UI',22,'bold'),padding=18).pack(anchor='w')
    ...
    status=tk.StringVar(value='Local image processing â€¢ editable solid brushes â€¢ CoD4 iwmap 4')
    ...
    for key,label,value,choices in [
        ...
        ('threshold','Threshold (0â€“255)','128',None),
    ...
    def convert():
        ...
        command=[worker,'-m','prefabdrop.app',str(state['source']),'-o',str(target),'--preview',str(picture)]
        ...
        status.set(f'{Path(state["source"]).name} â€¢ {report["brushes"]:,} brushes â€¢ depth {report["settings"]["depth"]:g} units â€¢ preview current')
        return target
    def guard(fn):
        def wrapped():
            try: fn()
            except Exception as e: messagebox.showerror('Image â†’ Radiant',str(e))
        return wrapped
    def load(path=None):
    ...
    for label,action in [('Open imageâ€¦',load),('Load honeycomb example',lambda:load(BASE/'examples'/'honeycomb.png')),('Update preview',convert),('Export .mapâ€¦',save)]:
